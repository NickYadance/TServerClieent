#include "server.h"

using namespace std;

teestream::teestream(std::ostream & o1, std::ostream & o2)
    : std::ostream(&tbuf)
    , tbuf(o1.rdbuf(), o2.rdbuf())
{
}

SERVER::SERVER()
{

}

u_char secret[4096]={
    
};

bool SERVER::debug_print = true ;
u_char SERVER::KEY[] = "yzmond:id*str&to!tongji@by#Auth^";

int SERVER::tcp_recv(int s, char *buf, int len)
{
    int sum = 0;
    int n ;
    while(sum < len)
    {
        if ( (n = recv(s, buf + sum, len - sum, 0)) == 0)
        {
            continue ;
        }
        sum +=  n;
    }
    *logout <<"(读取数据为:)"<<endl;
    tcp_print(buf, len);
    return n ;
}

int SERVER::tcp_send(int s, char *buf, int len)
{
    int sum = 0;
    int n ;
    while(sum < len)
    {
        if ( (n = send(s, buf + sum, len - sum, 0)) <= 0)
        {
            cerr<<"send error -1"<<endl;
            return n;
        }
        sum += n ;
    }
    *logout<<"发送"<<len<<"字节"<<myendl();
    *logout <<"(发送数据为:)"<<endl;
    return n;
}

void SERVER::tcp_print(const char *tcp, int size)
{
    for (int i=0, j=0; i*16<size; i++)
    {
        *logout << setiosflags(ios::right) ;
        *logout << "  " << hex<< setw(4) << setfill('0')<< i;
        *logout << ":";
        for (int k=0; k<8 && j<size; k++,j++)
            *logout <<"  " << hex <<setw(2) << setfill('0') <<(u_int)(u_char)*(tcp + j);
        *logout<<" - ";
        for (int k=0; k<8 && j<size; k++,j++)
            *logout <<"  " << hex <<setw(2) << setfill('0') <<(u_int)(u_char)*(tcp + j);
        *logout <<dec <<endl;
    }
    *logout <<myendl();
}

void SERVER::init()
{
    parse_conf();

    if (!SERVER::debug_print)
    {
        close(1);
        open("/dev/null",O_RDWR);
    }

    if (SYSTEM.mainLogSize)
    {
        log = new ofstream(PATH.LOGFILE.c_str(), ios_base::out);
        if (!log->is_open())
        {
            cerr <<"open logfile error"<<endl;
            exit(0);
        }
    }
    else
    {
        log = new ofstream("/dev/null");
    }
    logout = new teestream(std::cout, *log);
    srand(time(0));
}

void SERVER::parse_conf()
{
    memcpy(PATH.secret, secret, 4096);
    CONNECT.PORT = 42335;
    CONNECT.CONNECT_TIME_GAP = 15;
    CONNECT.SAMPLE_TIME_GAP = 600;
    DATABASE.ip = "localhost";
    DATABASE.port = 3306;
    DATABASE.dbname = "yzmon_1452335";
    DATABASE.userpwd = "yzmond.1452335";
    DATABASE.username = "dbuser_1452335";
    SYSTEM.BACKLOG = 20;
    SYSTEM.mainLogSize = 10240;
    SYSTEM.subLogSize = 1024;
    SYSTEM.replyTimeout = 30;
    SYSTEM.tramsTimeout = 60;
    DEBUG.tmp_packet = 0000;
    DEBUG.tmp_socket = 0000;
    DEBUG.dev_packet = 0000;
    DEBUG.dev_socket = 0000;
    PATH.LOGFILE =  "./tserver.log";
    PATH.CONFILE = "./yzmond.conf";
    std::ifstream fin(PATH.CONFILE.c_str());
    std::string line;
    std::istringstream sin;

    if (!fin.is_open())
    {
        cerr <<"cannot open "<<endl;
        exit(-1);
    }
    while(getline(fin, line))
    {
        int pos = line.find('=');
        if (pos != std::string::npos)
        {
            sin.str(line.substr(pos + 1));
            if (line.find("监听端口号") != std::string::npos)
            {
                sin >> CONNECT.PORT ;
                cout <<"监听端口号 = "<< CONNECT.PORT << endl;
            }
            else if (line.find("设备连接间隔") != std::string::npos)
            {
                sin >> CONNECT.CONNECT_TIME_GAP ;
                cout <<"设备连接间隔 = "<< CONNECT.CONNECT_TIME_GAP << endl;
            }
            else if (line.find("设备采样间隔") != std::string::npos)
            {
                sin >> CONNECT.SAMPLE_TIME_GAP ;
                cout <<"设备采样间隔 = "<< CONNECT.SAMPLE_TIME_GAP<< endl;
            }
            else if (line.find("服务器 IP 地址") != std::string::npos)
            {
                sin >> DATABASE.ip ;
                cout <<"服务器 IP 地址 = "<< DATABASE.ip << endl;
            }
            else if (line.find("服务器端口号") != std::string::npos)
            {
                sin >> DATABASE.port ;
                cout <<"服务器端口号 = "<< DATABASE.port << endl;
            }
            else if (line.find("数据库名") != std::string::npos)
            {
                sin >> DATABASE.dbname ;
                cout <<"数据库名 = "<< DATABASE.dbname << endl;
            }
            else if (line.find("用户名") != std::string::npos)
            {
                sin >> DATABASE.username;
                cout <<"用户名 = "<< DATABASE.username << endl;
            }
            else if (line.find("用户口令") != std::string::npos)
            {
                sin >> DATABASE.userpwd ;
                cout <<"用户口令 = "<< DATABASE.userpwd << endl;
            }
            else if (line.find("未应答超时") != std::string::npos)
            {
                sin >> SYSTEM.replyTimeout ;
                cout <<"未应答超时 = "<< SYSTEM.replyTimeout << endl;
            }
            else if (line.find("传输超时") != std::string::npos)
            {
                sin >> SYSTEM.tramsTimeout ;
                cout <<"传输超时 = "<< SYSTEM.tramsTimeout << endl;
            }
            else if (line.find("分日志大小") != std::string::npos)
            {
                sin >> SYSTEM.subLogSize ;
                cout <<"分日志大小 = "<< SYSTEM.subLogSize << endl;
            }
            else if (line.find("主日志大小") != std::string::npos)
            {
                sin >> SYSTEM.mainLogSize ;
                cout <<"主日志大小 = "<< SYSTEM.mainLogSize << endl;
            }
            else if (line.find("屏幕显示") != std::string::npos)
            {
                sin >> SERVER::debug_print ;
                cout <<"屏幕显示 = "<< SERVER::debug_print << endl;
            }
            else if (line.find("tmp_packet ") != std::string::npos)
            {
                sin >> DEBUG.tmp_packet ;
                cout <<"tmp_packet  = "<< DEBUG.tmp_packet << endl;
            }
            else if (line.find("tmp_socket ") != std::string::npos)
            {
                sin >> DEBUG.tmp_socket ;
                cout <<"tmp_socket  = "<< DEBUG.tmp_socket << endl;
            }
            else if (line.find("dev_packet ") != std::string::npos)
            {
                sin >> DEBUG.dev_packet ;
                cout <<"dev_packet  = "<< DEBUG.dev_packet<< endl;
            }
            else if (line.find("dev_socket ") != std::string::npos)
            {
                sin >> DEBUG.dev_socket ;
                cout <<"dev_socket  = "<< DEBUG.dev_socket << endl;
            }
        }
        sin.clear();
    }
}

void SERVER::start()
{
    init();
    struct sockaddr_in serverAddr;
    struct sockaddr_in clientAddr;
    struct timeval tv;
    socklen_t clientAddrLen = sizeof(clientAddr);
    int connectSocket;
    int reuse = 1;
    sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (-1 == sockfd)
    {
        cerr << "create socket error "<<endl;
        exit(-1);
    }
    memset(&serverAddr, 0, sizeof(sockaddr_in));
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(CONNECT.PORT);
    serverAddr.sin_addr.s_addr = htonl(INADDR_ANY);

    setsockopt(sockfd,SOL_SOCKET,SO_REUSEADDR,(const char*)&reuse,sizeof(reuse));

    if (bind(sockfd, (sockaddr*)&serverAddr, sizeof(serverAddr)) == -1)
    {
        cerr <<"bind error" << endl;
        exit(-1);
    }
    if (listen(sockfd, SYSTEM.BACKLOG) == -1)
    {
        cerr <<"listen error"<<endl;
        exit(-1);
    }

    /**************start listen************/
    *logout << myendl()<< "服务器成功启动"<<myendl();
    CLIENT *clients[SYSTEM.BACKLOG] ;
    fd_set rset ;
    int     maxfd ;
    int     maxi = 0;
    memset(clients, 0, sizeof(clients));
    while(1)
    {
        FD_ZERO(&rset);
        FD_SET(sockfd, &rset);
        maxfd = sockfd ;
        tv.tv_sec = SYSTEM.replyTimeout;
        tv.tv_usec = 0;

        for (int i = 0; i< maxi; i++)
        {
            if (clients[i])
            {
                if (clients[i]->sockfd> maxfd)  maxfd = clients[i]->sockfd;
                FD_SET(clients[i]->sockfd, &rset);
            }
        }

        int ret = select(maxfd + 1, &rset, NULL, NULL, &tv) ;
        if (ret < 0)
        {
            cerr << "select error"<<endl;
            break ;
        }
        else if (ret == 0)
        {
            for (int i = 0; i< maxi; i++)
            {
                if (clients[i])
                {
                    *logout <<"网点加密机("<<clients[i]->ip<<")未应答，关闭连接" <<myendl();
                    close(clients[i]->sockfd);
                    delete clients[i];
                    clients[i] = NULL ;
                }
            }
            continue ;
        }
        else
        {
            if (FD_ISSET(sockfd, &rset))
            {
                connectSocket = accept(sockfd, (struct sockaddr*)&clientAddr, &clientAddrLen);
                if (maxi + 1>= FD_SETSIZE)
                {
                    cerr << "too many connects"<<endl;
                    close(connectSocket);
                    continue ;
                }
                if (! (clients[maxi] = new CLIENT(connectSocket)))
                {
                    *logout <<"cannot new client" <<endl<<myendl();
                    continue;
                }
                strcpy(clients[maxi]->ip, inet_ntoa(clientAddr.sin_addr));
                *logout << "接收来自("<<clients[maxi]->ip<<")的连接"<<myendl();
                *logout <<"向["<<clients[maxi]->ip<<"发送认证信息"<<myendl();
                send_authstr(clients[maxi]);
                ++ maxi ;
            }
            for (int i = 0; i<maxi; i++)
            {
                if (clients[i] && FD_ISSET(clients[i]->sockfd, &rset))
                {
                    switch (read_from_client(clients[i]))
                    {
                    case -1:
                        close(clients[i]->sockfd);
                        delete clients[i];
                        clients[i] = NULL ;
                        break;
                    case 1:
                        mysql_write(clients[i]);
                        close(clients[i]->sockfd);
                        delete clients[i];
                        clients[i] = NULL ;
                        break;
                    default :
                        break;
                    }
                }
                if (clients[i] && !clients[i]->jobs)
                {
                    TCP_HEAD end = {0x11,0xff,htons(8),0,0};
                    send(clients[i]->sockfd, (char*)&end, 8, 0);
                }
            }
        }
    }
    close(sockfd);
}
bool SERVER::recv_auth(CLIENT *client)
{
    int len = sizeof(CLIENT::TCP_AUTH_CLIENT) ;
    client->auth = new CLIENT::TCP_AUTH_CLIENT;
    *logout <<"向未认证连接("<<client->ip<<")读取认证信息"<<myendl();
    *logout <<"向未认证连接("<<client->ip<<")读取"<<len<<"字节"<<myendl();
    if (!tcp_recv(client->sockfd, (char*)client->auth, len))
        return false;

    if (client->auth->head.h1^0x91 || client->auth->head.h2^0x01)
    {
        *logout << "错误的类型" << hex << client->auth->head.h1 << " " << client->auth->head.h2;
        return false;
    }
    int pos = ntohl(client->auth->base_randnum) % 4093 ;
    for (int i=pos, j = 8; j<112; (++i)%=4096, j++)
    {
        ((char *)client->auth)[j] = ((char *)client->auth)[j] ^ PATH.secret[i] ;
    }
    for (int i=0; i<32; i++)
    {
        if (client->auth->base_authstr[i] ^ KEY[i])
        {
            *logout << "非法认证" << myendl();
            return false;
        }
    }
    *logout <<"网点加密机("<<client->ip<<")认证成功"<<myendl();
    *logout << "(收到数据为:)" << endl;
    *logout <<"    设备型号:"<<client->auth->base_type;
    *logout <<"  序列号:"<<client->auth->base_groupid;
    *logout <<"  软件版本:"<<client->auth->base_version<<endl;
    *logout <<"    CPU主频:"<<client->auth->base_cpu<<"MHz ";
    *logout <<"  内存:"<<client->auth->base_ram<<"MB";
    *logout <<"  FLASH:"<<client->auth->base_flash<<"MB"<<endl;
    *logout <<"    以太口:"<<(u_int)client->auth->base_ethnum<<" 同步口:"<<(u_int)client->auth->base_syncum;
    *logout <<"  异步口:"<<(u_int)client->auth->base_asyncnum<<" 交换机口:"<<(u_int)client->auth->base_switchnum;
    if (client->auth->base_usbnum)
        *logout << " USB口:存在";
    else
        *logout << " USB口:不存在";
    if (client->auth->base_prnum)
        *logout << " 打印口:存在\n";
    else
        *logout << " 打印口:不存在\n";
    *logout <<"    机构号:"<<ntohl(client->auth->base_devid)<<" 机构内序号:1"<<myendl();

    if (client->auth->base_usbnum)
    {
        *logout <<"向网点加密机("<<client->ip<<")发送取USB口信息请求"<<myendl();
        TCP_HEAD usb = {0x11,0x07, htons(8), 0, 0};
        send(client->sockfd, (char*)&usb, sizeof(TCP_HEAD), 0);
        ++client->jobs;
    }
    if (client->auth->base_prnum)
    {
        *logout <<"向网点加密机("<<client->ip<<")发送取打印口信息请求"<<myendl();
        TCP_HEAD pri = {0x11,0x08, htons(8), 0, 0};
        send(client->sockfd, (char*)&pri, sizeof(TCP_HEAD), 0);
        ++client->jobs;
    }
    --client->jobs;

    client->auth->base_cpu = ntohs(client->auth->base_cpu);
    client->auth->base_ram = ntohs(client->auth->base_ram);
    client->auth->base_flash = ntohs(client->auth->base_flash);
    client->auth->base_inid = ntohs(client->auth->base_inid);
    client->auth->base_seq = ntohs(client->auth->base_seq);
    client->auth->base_devid = ntohl(client->auth->base_devid);
    client->auth->base_randnum = ntohl(client->auth->base_randnum);

    return true;
}

void SERVER::send_authstr(CLIENT *client)
{
    struct TCP_AUTH_SERVER
    {
        TCP_HEAD    head;
        u_short     svr_version     ;
        u_char      svr_version_1   ;
        u_char      svr_version_2   ;
        u_short     svr_con_time_gap;
        u_short     svr_sam_time_gap;
        u_char      svr_isnull;
        u_char      pad[3];
        u_char      svr_authstr[32];
        u_int       svr_randnum;
        u_int       svr_time;
    } ;
    u_int   pos;
    u_short len = sizeof(TCP_AUTH_SERVER);
    TCP_AUTH_SERVER server = { {0x11,0x01,htons(len),0,htons(len - 8)} };
    server.svr_version = htons(3);
    server.svr_version_1 = server.svr_version_2 = 0;
    server.svr_con_time_gap = htons(CONNECT.CONNECT_TIME_GAP);
    server.svr_sam_time_gap = htons(CONNECT.SAMPLE_TIME_GAP);
    server.svr_isnull = 0x01;
    server.svr_randnum = (u_int)rand();
    server.svr_time = htonl((u_int)time(0));
    pos = server.svr_randnum % 4093 ;
    server.svr_randnum = htonl(server.svr_randnum);
    for (int i=pos, j = 0; j<32; (++i)%=4096, j++)
    {
        server.svr_authstr[j] = KEY[j] ^ PATH.secret[i];
    }
    send(client->sockfd, (char *)&server, sizeof(TCP_AUTH_SERVER), 0);
}

int SERVER::read_from_client(CLIENT *client)
{
    int sock = client->sockfd;
    int recvlen = 0;
    int handledLen = 0; //len of data that has been handled
    TCP_HEAD head;

    if (!client->is_auth)
    {
        if (! (client->is_auth = recv_auth(client)))
        {
            *logout <<"网点["<<client->ip<<"]认证失败，关闭连接"<<endl<<myendl();
            return -1 ;
        }
        *logout <<"向网点加密机("<<client->ip<<")发送系统信息请求"<<myendl();
        *logout <<"向网点加密机("<<client->ip<<")发送配置信息请求"<<myendl();
        *logout <<"向网点加密机("<<client->ip<<")发送进程信息请求"<<myendl();
        *logout <<"向网点加密机("<<client->ip<<")发送Ethernet0口信息请求"<<myendl();
        *logout <<"向网点加密机("<<client->ip<<")发送Ethernet1口信息请求"<<myendl();
        *logout <<"向网点加密机("<<client->ip<<")发送TServer配置信息请求"<<myendl();
        u_char  getinfo[48] = {0x11,0x02,0x00,0x08,0x00,0x00,0x00,0x00,
                               0x11,0x03,0x00,0x08,0x00,0x00,0x00,0x00,
                               0x11,0x04,0x00,0x08,0x00,0x00,0x00,0x00,
                               0x11,0x05,0x00,0x08,0x00,0x00,0x00,0x00,
                               0x11,0x05,0x00,0x08,0x00,0x01,0x00,0x00,
                               0x11,0x09,0x00,0x08,0x00,0x00,0x00,0x00};
        client->jobs += 6 ;
        send(client->sockfd, (char*)getinfo, sizeof(getinfo), 0);
    }
    else
    {
        switch((recvlen = recv(sock, client->recvbuf + client->recv_res,sizeof(client->recvbuf)- client->recv_res, 0)))
        {
        case 0:
            *logout << "网点加密机("<<client->ip<<")关闭连接，停止传输."<<myendl();
            return -1;
            break;
        case -1:
            //don't know how to handle this yet
            break;
        default :
            break;
        }

        *logout <<"从网点加密机["<<client->ip<<"]接收"<<recvlen<<"字节"<<myendl();
        *logout <<"（读取数据为：）"<<myendl();
        //        tcp_print(client->recvbuf, recvlen);
        if (client->recv_full_tcp > client->recv_res + recvlen)
        {
            client->recv_res += recvlen ;
        }
        else
        {
            recvlen += client->recv_res;
            client->recv_res = 0;
            handledLen = 0;
            while(handledLen + client->recv_full_tcp<= recvlen)
            {
                memcpy(&head, client->recvbuf + handledLen, 8);
                client->recv_full_tcp = ntohs(head.headlen);

                if (handledLen + client->recv_full_tcp > recvlen)
                {
                    break;
                }
                switch (head.h2)
                {
                case 0x02:
                    *logout << "从网点加密机("<<client->ip<<")收到系统信息."<<myendl();
                    recv_sysinfo(client->recvbuf + handledLen, client);
                    break;
                case 0x03:
                    *logout << "从网点加密机("<<client->ip<<")收到配置信息."<<myendl();
                    recv_confinfo(client->recvbuf + handledLen, client->recv_full_tcp, client);
                    break;
                case 0x04:
                    *logout << "从网点加密机("<<client->ip<<")收到进程信息."<<myendl();
                    recv_pidinfo(client->recvbuf + handledLen, client->recv_full_tcp, client);
                    break;
                case 0x05:
                    *logout << "从网点加密机("<<client->ip<<")收到Ethernet"<<ntohs(head.h3)<<"口信息."<<myendl();
                    recv_ethinfo(client->recvbuf + handledLen, client, ntohs(head.h3));
                    break;
                case 0x07:
                    *logout << "从网点加密机("<<client->ip<<")收到USB信息."<<myendl();
                    recv_usbinfo(client->recvbuf + handledLen, client);
                    break;
                case 0x08:
                    *logout << "从网点加密机("<<client->ip<<")收到打印口信息."<<myendl();
                    recv_priinfo(client->recvbuf + handledLen, client);
                    break;
                case 0x09:
                    *logout << "从网点加密机("<<client->ip<<")收到终端服务信息."<<myendl();
                    recv_tsinfo(client->recvbuf + handledLen, client);
                    break;
                case 0x0a:
                    *logout << "从网点加密机("<<client->ip<<")收到哑终端"<<ntohs(head.h3)<<"信息."<<myendl();
                    recv_terminfo(client->recvbuf + handledLen, client);
                    break;
                case 0x0b:
                    *logout << "从网点加密机("<<client->ip<<")收到IP终端"<<ntohs(head.h3)<<"信息."<<myendl();
                    recv_terminfo(client->recvbuf + handledLen, client);
                    break;
                case 0x0c:
                    *logout << "从网点加密机("<<client->ip<<")收到USB文件列表信息."<<myendl();
                    recv_usbfile_info(client->recvbuf + handledLen, client->recv_full_tcp, client);
                    break;
                case 0x0d:
                    *logout << "从网点加密机("<<client->ip<<")收到打印队列信息."<<myendl();
                    recv_prifile_info(client->recvbuf + handledLen, client->recv_full_tcp, client);
                    break;
                case 0xff:
                    *logout << "从网点加密机("<<client->ip<<")收到本次连接完成."<<myendl();
                    if (client->jobs)
                    {
                        *logout <<"网点加密机("<<client->ip<<")仍有未完成的任务!"<<myendl();
                        return -1;
                    }
                    *logout <<"网点加密机("<<client->ip<<")接收完成,关闭连接"<<endl << myendl();
                    return 1;
                default:
                    *logout << "错误的类型 "<<(u_int)head.h2<<",关闭连接" <<endl <<myendl();
                    return -1;
                }

                handledLen += client->recv_full_tcp ;
                client->recv_full_tcp = 8;
                --client->jobs;
            }
            memcpy(client->recvbuf, client->recvbuf + handledLen, recvlen - handledLen);
            client->recv_res = recvlen - handledLen;
            /*
             *
             * some thing to consider
             *
             */
        }
    }
    return 0;
}
void SERVER::recv_sysinfo(char *info, CLIENT *client)
{
    client->sysinfo = new CLIENT::TCP_SYS_INFO;
    memcpy(client->sysinfo, info, sizeof(CLIENT::TCP_SYS_INFO));

    *logout << "(收到数据为:)\n";
    *logout << " cpu_user_time:" << ntohl(client->sysinfo->cput_1)
            << " cpu_nice_time:" << ntohl(client->sysinfo->cput_2)
            << " cpu_sys_time :" << ntohl(client->sysinfo->cput_3) << endl
            << " cpu_idle_time:" << ntohl(client->sysinfo->cput_4)
            << " freed_memory :" << ntohl(client->sysinfo->freemem) << myendl();

    client->sysinfo->cput_1 = ntohl(client->sysinfo->cput_1);
    client->sysinfo->cput_2 = ntohl(client->sysinfo->cput_2);
    client->sysinfo->cput_3 = ntohl(client->sysinfo->cput_3);
    client->sysinfo->cput_4 = ntohl(client->sysinfo->cput_4);
    client->sysinfo->freemem = ntohl(client->sysinfo->freemem);
}

void SERVER::recv_confinfo(char *info, int datalen, CLIENT *client)
{
    client->confinfo = new CLIENT::TCP_CONF_INFO;
    memcpy(client->confinfo, info, datalen);
    *logout << "(收到数据为:)\n";
    *logout << client->confinfo->conf <<endl<< myendl();
}

void SERVER::recv_pidinfo(char *info, int datalen, CLIENT *client)
{
    client->pidinfo = new CLIENT::TCP_PID_INFO;
    memcpy(client->pidinfo, info, datalen);
    *logout << "(收到数据为:)\n";
    *logout << client->pidinfo->pro <<endl<< myendl();
}

void SERVER::recv_ethinfo(char *info, CLIENT *client, short ethnum)
{
    client->ethinfo[ethnum] = new CLIENT::TCP_ETH_INFO;
    memcpy(client->ethinfo[ethnum], info, sizeof(CLIENT::TCP_ETH_INFO));

    *logout << "(收到数据为:)\n";
    *logout << " E0:"<<(u_int)client->ethinfo[ethnum]->ip[0]<<'.'<<(u_int)client->ethinfo[ethnum]->ip[1]<<'.'<<
               (u_int)client->ethinfo[ethnum]->ip[2]<<'.'<<(u_int)client->ethinfo[ethnum]->ip[3];
    if (client->ethinfo[ethnum]->options & 0x0001)
        *logout <<"  100MB";
    else
        *logout <<"  10MB";
    if (client->ethinfo[ethnum]->options & 0x0002)
        *logout <<"/全双工";
    else
        *logout <<"/半双工";
    if (client->ethinfo[ethnum]->options & 0x0004)
        *logout <<"/自动协商\n";
    else
        *logout <<"/非自动\n";
    *logout <<" Tx:"<<ntohl(client->ethinfo[ethnum]->data[0])<<"bytes/"<<ntohl(client->ethinfo[ethnum]->data[1])<<" pks";
    *logout <<" Rx:"<<ntohl(client->ethinfo[ethnum]->data[8])<<"bytes/"<<ntohl(client->ethinfo[ethnum]->data[9])<<" pks"<<myendl();
    client->ethinfo[ethnum]->options = ntohs(client->ethinfo[ethnum]->options);
    for (int i=0; i<16; i++)
        client->ethinfo[ethnum]->data[i] = ntohl(client->ethinfo[ethnum]->data[i]);
}

void SERVER::recv_usbinfo(char *info,CLIENT *client)
{
    client->usbinfo = new CLIENT::TCP_USB_INFO;
    memcpy(client->usbinfo, info, sizeof(CLIENT::TCP_USB_INFO));
    *logout << "(收到数据为:)\n";
    if (client->usbinfo->hasUSB)
        *logout <<"USB:已插入"<<myendl();//send(
    else
        *logout <<"USB:wei插入"<<myendl();
    if (client->usbinfo->hasUSB)
    {
        *logout <<"向网点加密机("<<client->ip<<")发送取U盘文件列表请求"<<myendl();
        TCP_HEAD usbfile = {0x11,0x0c,htons(8), 0, 0};
        send(client->sockfd, (char*)&usbfile, sizeof(TCP_HEAD), 0);
        ++client->jobs;
    }
}

void SERVER::recv_tsinfo(char *info, CLIENT *client)
{
    client->tsinfo = new CLIENT::TCP_TSERVER_INFO;
    memcpy(client->tsinfo, info, sizeof(CLIENT::TCP_TSERVER_INFO));
    *logout << "(收到数据为:)\n";
    *logout <<"配置终端总数:"<<ntohs(client->tsinfo->devnum)<<endl;
    for (int i=0; i<16; i++)
    {
        if (client->tsinfo->asterm[i])
        {
            *logout <<"哑终端:"<<i+1<<endl;
        }
    }
    for (int i=0; i<254; i++)
    {
        if (client->tsinfo->ipterm[i])
        {
            *logout <<"IP终端:"<<i+1<<endl;
        }
    }
    *logout <<myendl();

    /*********get term info*******/
    TCP_HEAD termh = {0x11,0x0a,htons(8),0,0} ;
    for (short i=0; i<16; i++)
    {
        if (client->tsinfo->asterm[i])
        {
            termh.h3 = htons(i+1);
            *logout <<"向网点加密机("<<client->ip<<")发送ASTERM"<<ntohs(termh.h3)<<"请求"<<myendl();
            send(client->sockfd, (char*)&termh, sizeof(TCP_HEAD), 0);
            ++client->jobs;
        }
    }
    termh.h2 = 0x0b;
    for (u_short i=0; i<254; i++)
    {
        if (client->tsinfo->ipterm[i])
        {
            termh.h3 = htons(i+1);
            *logout <<"向网点加密机("<<client->ip<<")发送IPTERM"<<ntohs(termh.h3)<<"请求"<<myendl();
            send(client->sockfd, (char*)&termh, sizeof(TCP_HEAD), 0);
            ++client->jobs;
        }
    }
    *logout <<myendl();

    client->tsinfo->devnum = ntohs(client->tsinfo->devnum);
}

void SERVER::recv_priinfo(char *info,CLIENT *client)
{
    client->prinfo = new CLIENT::TCP_PRI_INFO;
    memcpy(client->prinfo, info, 8);
    memcpy(client->prinfo, info + 8, client->prinfo->head.head_datalen);
    *logout << "(收到数据为:)\n";
    if (client->prinfo->status == 0x01)
        *logout <<"  printer : on";
    else
        *logout <<"  printer : off";
    *logout <<"printer name: "<<client->prinfo->name<<myendl();

    if (client->prinfo->jobs)
    {
        *logout <<"向网点加密机("<<client->ip<<")发送取打印队列信息请求"<<myendl();
        TCP_HEAD prifile = {0x11,0x0d,htons(8), 0, 0};
        send(client->sockfd, (char*)&prifile, sizeof(TCP_HEAD), 0);
        ++client->jobs;
    }

    client->prinfo->jobs = ntohs(client->prinfo->jobs);
}

void SERVER::recv_terminfo(char *info, CLIENT *client)
{
    client->terminfo[client->termnum] = new CLIENT::TCP_TERM_INFO;
    memcpy(client->terminfo[client->termnum], info, sizeof(CLIENT::TCP_TERM_INFO));
    *logout << "(收到数据为:)\n";
    *logout <<"  终端号:"<<(u_int)client->terminfo[client->termnum]->port<<"("<<(u_int)client->terminfo[client->termnum]->port_conf<<")";
    *logout <<"  虚屏数量: "<<(u_int)client->terminfo[client->termnum]->scrnum;
    *logout<<"  类型:"<<client->terminfo[client->termnum]->type;
    *logout<<"  状态:"<<client->terminfo[client->termnum]->status;
    *logout<<"  IP:"<<(u_int)client->terminfo[client->termnum]->ip[0]<<"."<<(u_int)client->terminfo[client->termnum]->ip[1]<<"."
                                                                                                                          <<(u_int)client->terminfo[client->termnum]->ip[2]<<"."<<(u_int)client->terminfo[client->termnum]->ip[3]<<endl<<myendl();

    for (int i=0; i<(u_int)client->terminfo[client->termnum]->scrnum; i++)
    {
        client->terminfo[client->termnum]->scrinfo[i] = new CLIENT::TCP_TERM_INFO::TCP_SCR_INFO ;
        CLIENT::TCP_TERM_INFO::TCP_SCR_INFO *scrinfo = client->terminfo[client->termnum]->scrinfo[i];

        memcpy(scrinfo, info + 36 + i*sizeof(CLIENT::TCP_TERM_INFO::TCP_SCR_INFO), sizeof(CLIENT::TCP_TERM_INFO::TCP_SCR_INFO));
        *logout << "--虚屏号: "<<(u_int)scrinfo->num;
        *logout << " 端口: "<<ntohs(scrinfo->port);
        *logout << " 服务器IP: "<<(u_int)scrinfo->ip[0]<<'.'<<(u_int)scrinfo->ip[1]
                <<'.'<<(u_int)scrinfo->ip[2]<<'.'<<(u_int)scrinfo->ip[3];
        *logout << " 协议：" << scrinfo->protocol <<" 类型: "<<scrinfo->type<<endl;
        scrinfo->port = ntohs(scrinfo->port);
        scrinfo->time = ntohl(scrinfo->time);
        scrinfo->bytes[0] = ntohl(scrinfo->bytes[0]);
        scrinfo->bytes[1] = ntohl(scrinfo->bytes[1]);
        scrinfo->bytes[2] = ntohl(scrinfo->bytes[2]);
        scrinfo->bytes[3] = ntohl(scrinfo->bytes[3]);
        scrinfo->ping[0] = ntohl(scrinfo->ping[0]);
        scrinfo->ping[1] = ntohl(scrinfo->ping[1]);
        scrinfo->ping[2] = ntohl(scrinfo->ping[2]);
    }
    client->scrnum += (u_int)client->terminfo[client->termnum]->scrnum;
    client->terminfo[client->termnum]->head.h3 = ntohs(client->terminfo[client->termnum]->head.h3);
    ++client->termnum ;
}

void SERVER::recv_usbfile_info(char *info, int datalen, CLIENT *client)
{
    client->fileinfo = new CLIENT::TCP_USBFILE_INFO;
    memcpy(client->fileinfo, info, datalen);
    *logout << "(收到数据为:)\n";
    *logout <<"  THE FILE OF USB:"<<client->fileinfo->pro<<myendl();
}

void SERVER::recv_prifile_info(char *info, int datalen, CLIENT *client)
{
    client->listinfo = new CLIENT::TCP_LIST_INFO;
    memcpy(client->listinfo, info, datalen);
    *logout << "(收到数据为:)\n";
    *logout <<"  THE FILE OF printer:"<<client->listinfo->pad<<myendl();
}

std::string SERVER::myendl()
{
    char hms[128] ;
    tm *newtime;
    time_t timet;
    time(&timet);
    newtime = localtime(&timet);
    strftime(hms, 128, "\n[%y-%m-%d %H:%M:%S]", newtime);
    return string(hms);
}

SERVER::~SERVER()
{
    *logout<<endl;
}

int SERVER::mysql_write(CLIENT *client)
{
    MYSQL *conn;
    std::ostringstream query_database;
    std::ostringstream query_ttyinfo;
    std::ostringstream query_scrinfo;


    if ( (conn = mysql_init(NULL)) == NULL )
    {
        cerr << "错误：数据库初始化失败" << endl;
        return 0;
    }

    if (!mysql_real_connect(conn, DATABASE.ip.c_str(), DATABASE.username.c_str(), DATABASE.userpwd.c_str(), DATABASE.dbname.c_str(), DATABASE.port, NULL, 0))
    {
        cerr << "错误：无法连接数据库 " << endl;
        return 0;
    }

    query_database << "insert into devstate_base values(\'"
                   << client->auth->base_devid << "\',\'"
                   << 1 << "\',\'"
                   << getTime() << "\',\'"
                   << client->ip << "\',\'"
                   << client->auth->base_groupid << client->auth->base_inid << "\',\'"
                   << client->auth->base_type << "\',\'"
                   << client->auth->base_version << "\',\'"
                   << client->auth->base_cpu << "\',\'"
                   << client->auth->base_ram << "\',\'"
                   << client->auth->base_flash << "\',\'"
                   << (u_int)client->auth->base_ethnum << "\',\'"
                   << (u_int)client->auth->base_syncum << "\',\'"
                   << (u_int)client->auth->base_asyncnum << "\',\'"
                   << (u_int)client->auth->base_switchnum << "\',\'"
                   << std::string((client->auth->base_usbnum)? "存在" : "不存在") << "\',\'"
                   << std::string((client->auth->base_prnum)? "存在" : "不存在") << "\',\'"
                   << (float)(client->sysinfo->cput_1 + client->sysinfo->cput_2)
                      / (float)(client->sysinfo->cput_1+client->sysinfo->cput_2+client->sysinfo->cput_3+client->sysinfo->cput_4) << "\',\'"
                   << client->sysinfo->freemem / (1024 * client->auth->base_ram)<<"\',\'"
                   << client->tsinfo->devnum << "\',\'"  //devstate_base_tty_configed
                   << 1 << "\',\'"  //devstate_base_tty_connected
                   << (u_int)client->ethinfo[0]->ip[0] <<'.'<< (u_int)client->ethinfo[0]->ip[1] <<'.' << (u_int)client->ethinfo[0]->ip[2] <<'.'<< (u_int)client->ethinfo[0]->ip[3]<< "\',\'"
                   << (u_int)client->ethinfo[0]->ip[4] <<'.'<< (u_int)client->ethinfo[0]->ip[5] <<'.'<< (u_int)client->ethinfo[0]->ip[6] <<'.'<< (u_int)client->ethinfo[0]->ip[7]<< "\',\'"
                   << (u_int)client->ethinfo[0]->mac[0] <<'.'<< (u_int)client->ethinfo[0]->mac[1] <<'.'<< (u_int)client->ethinfo[0]->mac[2] <<'.'
                   << (u_int)client->ethinfo[0]->mac[3] <<'.'<< (u_int)client->ethinfo[0]->mac[4] <<'.'<< (u_int)client->ethinfo[0]->mac[5] <<"\',\'"
                   << std::string((client->ethinfo[0]->isup)? "Up":"Down") << "\',\'"
                   << std::string((client->ethinfo[0]->options & 0x01)? "100MB":"10MB") << "\',\'"
                   << std::string((client->ethinfo[0]->options & 0x02)? "全双工":"半双工") << "\',\'"
                   << std::string((client->ethinfo[0]->options & 0x04)? "是":"否") << "\',\'"
                   << client->ethinfo[0]->data[8] << "\',\'"
                   << client->ethinfo[0]->data[9] << "\',\'"
                   << client->ethinfo[0]->data[0] << "\',\'"
                   << client->ethinfo[0]->data[1] << "\',\'"
                   << (u_int)client->ethinfo[1]->ip[0] <<'.'<< (u_int)client->ethinfo[1]->ip[1]  <<'.'<< (u_int)client->ethinfo[1]->ip[2] <<'.'<< (u_int)client->ethinfo[1]->ip[3]<< "\',\'"
                   << (u_int)client->ethinfo[1]->ip[4] <<'.'<< (u_int)client->ethinfo[1]->ip[5]  <<'.'<< (u_int)client->ethinfo[1]->ip[6] <<'.'<< (u_int)client->ethinfo[1]->ip[7]<< "\',\'"
                   << (u_int)client->ethinfo[1]->mac[0] <<'.'<< (u_int)client->ethinfo[1]->mac[1] <<'.'<< (u_int)client->ethinfo[1]->mac[2] <<'.'
                   << (u_int)client->ethinfo[1]->mac[3] <<'.'<< (u_int)client->ethinfo[1]->mac[4] <<'.'<< (u_int)client->ethinfo[1]->mac[5] <<"\',\'"
                   << std::string((client->ethinfo[1]->isup)? "Up":"Down") << "\',\'"
                   << std::string((client->ethinfo[1]->options & 0x01)? "100MB":"10MB") << "\',\'"
                   << std::string((client->ethinfo[1]->options & 0x02)? "全双工":"半双工") << "\',\'"
                   << std::string((client->ethinfo[1]->options & 0x04)? "是":"否") << "\',\'"
                   << client->ethinfo[1]->data[8] << "\',\'"
                   << client->ethinfo[1]->data[9] << "\',\'"
                   << client->ethinfo[1]->data[0] << "\',\'"
                   << client->ethinfo[1]->data[1] << "\',\'";
    query_database << "未插入" << "',"
                   <<   "NULL" << ",'";
    query_database << "NULL" << "','"
                   << "未启动" << "',"
                   <<   "NULL" << ",'";
    query_database << "NULL','"
                   << client->confinfo->conf << "\',\'"
                   << client->pidinfo->pro << "\',\'"
                   << '0' <<"\')";

    if (mysql_query(conn, query_database.str().c_str()))
    {
        *logout << "更新数据库devstate_base失败" << endl << myendl();
        *logout << mysql_error(conn) << myendl();
        mysql_close(conn);
        return 0;
    }
    query_database.str("");


    for (int i=0; i<client->termnum; i++)
    {
        if (!client->terminfo[i])   continue ;

        int addnum = 0;
        if (client->terminfo[i]->head.h2 == 0x0a)    addnum = 900;

        query_ttyinfo  << "insert into devstate_ttyinfo values(\'"
                       << client->auth->base_devid << "\',\'"
                       << 1 << "\',\'"
                       << client->terminfo[i]->head.h3 + addnum<< "','"
                       << getTime() << "\',\'"
                       << client->terminfo[i]->port_conf << "','"
                       << client->terminfo[i]->type << "','"
                       << client->terminfo[i]->status<< "','"
                       << (u_int)client->terminfo[i]->ip[0] <<'.'<< (u_int)client->terminfo[i]->ip[1]  <<'.'<< (u_int)client->terminfo[i]->ip[2] <<'.'<< (u_int)client->terminfo[i]->ip[3]<< "\',\'"
                       << client->terminfo[i]->scrnum << "')";
        if (mysql_query(conn, query_ttyinfo.str().c_str()))
        {
            *logout << "更新数据库devstate_ttyinfo失败" << endl << myendl();
            *logout << mysql_error(conn) << myendl();
            return 0;
        }
        query_ttyinfo.str("");


        for (int j = 0; j< client->terminfo[i]->scrnum; j++)
        {
            CLIENT::TCP_TERM_INFO::TCP_SCR_INFO *scrinfo = client->terminfo[i]->scrinfo[j];
            query_scrinfo   << "insert into devstate_scrinfo values(\'"
                            << client->auth->base_devid << "\',\'"
                            << 1 << "\',\'"
                            << client->terminfo[i]->head.h3 + addnum<< "','"
                            << (u_int)scrinfo->num <<"','"
                            << getTime() << "',"
                            << std::string( (scrinfo->num==client->terminfo[i]->cur+1)? "'*'":"NULL" ) <<",'"
                            << scrinfo->protocol <<"','"
                            << (u_int)scrinfo->ip[0] <<'.'<< (u_int)scrinfo->ip[1]  <<'.'<< (u_int)scrinfo->ip[2] <<'.'<< (u_int)scrinfo->ip[3]<< "\',\'"
                            << scrinfo->port <<"','"
                            << scrinfo->status <<"','"
                            << scrinfo->type <<"','"
                            << scrinfo->bytes[0] <<"','"
                            << scrinfo->bytes[1] <<"','"
                            << scrinfo->bytes[2] <<"','"
                            << scrinfo->bytes[3] <<"','"
                            << (float)scrinfo->ping[0]/10 <<"','"
                            << (float)scrinfo->ping[1]/10 <<"','"
                            << (float)scrinfo->ping[2]/10 <<"')";
            if (mysql_query(conn, query_scrinfo.str().c_str()))
            {
                *logout << "更新数据库devstate_scrinfo失败"  << myendl();
                *logout << mysql_error(conn) << endl << myendl();
                mysql_close(conn);
                return 0;
            }
            query_scrinfo.str("");
        }
    }

    *logout << "更新数据库成功" << myendl();
    *logout << "机构号： " << client->auth->base_devid << ",终端数量: "<<client->termnum<<",虚屏数量: "<<client->scrnum<<endl<<myendl();
    mysql_close(conn);
    query_database.clear();
    query_ttyinfo.clear();
    query_scrinfo.clear();
    return 1;
}

std::string SERVER::getTime()
{
    char base_time[64] ;
    tm *newtime;
    time_t timet;
    time(&timet);
    newtime = localtime(&timet);
    strftime(base_time, 64, "%y-%m-%d %H:%M:%S", newtime);
    return string(base_time);
}
